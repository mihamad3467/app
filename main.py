...
async def prayer_for_location(latitude: float, longitude: float) -> tuple[str, dict[str, str]]:
    ...
    city = data.get("meta", {}).get("timezone", "موقعك").split("/")[-1].replace("_", " ")
    return city, timings
...
async def location_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        city, timings = await prayer_for_location(location.latitude, location.longitude)
        city = await reverse_geocode(location.latitude, location.longitude) or city
        with db() as connection:
            connection.execute(
                "UPDATE users SET lat=?, lng=?, city=? WHERE user_id=?",
                (location.latitude, location.longitude, city, user.id),
            )
        prayer_cache[user.id] = (date.today(), timings)
        text = (
...
async def prayer_alert_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with db() as connection:
        users = connection.execute(
            "SELECT * FROM users WHERE azan_enabled = 1 AND notifications_enabled = 1 AND lat IS NOT NULL"
        ).fetchall()
    for user in users:
        try:
            cached = prayer_cache.get(user["user_id"])
            if not cached or cached[0] != now.date():
                _, timings = await prayer_for_location(user["lat"], user["lng"])
